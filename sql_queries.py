...
(songplay_id bigint GENERATED BY DEFAULT AS IDENTITY(0,1) primary key, start_time timestamp references d_time (start_time), user_id int references d_users (user_id), level varchar, song_id varchar references d_songs(song_id), artist_id varchar references d_artists (artist_id) , session_id int not null , location varchar, user_agent varchar)
""")
...
songplay_table_insert = (""" insert into f_songplays (start_time, user_id, level, song_id,artist_id, session_id, location, user_agent)
                              select
                              TO_CHAR(TO_TIMESTAMP(se.ts / 1000), 'DD/MM/YYYY HH24:MI:SS') as start_time,
