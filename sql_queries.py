...
(songplay_id bigint GENERATED BY DEFAULT AS IDENTITY(0,1) primary key, start_time timestamp references d_time (start_time), user_id int references d_user (user_id), level varchar, song_id varchar references d_songs(song_id), artist_id varchar references d_artists (artist_id) , session_id int not null , location varchar, user_agent varchar)
""")
